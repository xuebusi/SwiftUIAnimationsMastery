    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            RoundedRectangle(cornerRadius: 20)
                .fill(Color.blue)
                .overlay(Text("Scale from top").foregroundStyle(.white))
                .scaleEffect(change ? 1 : 0.25, anchor: .top) // Add anchor location
                .animation(.default, value: change)
            
            RoundedRectangle(cornerRadius: 20)
                .foregroundStyle(.blue)
                .overlay(Text("Scale from right").foregroundStyle(.white))
                .scaleEffect(change ? 0.25 : 1, anchor: .trailing)
                .animation(.default, value: change)
            
            RoundedRectangle(cornerRadius: 20)
                .foregroundStyle(.blue)
                .overlay(Text("Scale from bottom left").foregroundStyle(.white))
